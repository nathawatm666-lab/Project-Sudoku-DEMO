...
public class SudokuGame extends JFrame {
    ...
    private int[][] puzzle = new int[SIZE][SIZE]; // เก็บโจทย์เริ่มต้น (ที่เจาะรู/ลบตัวเลขออกไปแล้วบางส่วน)
    private int[][] playerBoard = new int[SIZE][SIZE]; // เก็บตัวเลขปัจจุบันที่อยู่บนหน้าจอ (รวมที่โปรแกรมให้มา
                                                       // และที่ผู้เล่นพิมพ์ลงไป)
    private boolean[][] fixed = new boolean[SIZE][SIZE]; // อาเรย์บอกสถานะว่าช่องไหนเป็น "โจทย์ที่ห้ามแก้" (true =
    ...
    private void highlightCells() {
        for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                ...
                // ถ้าช่องนี้ใส่เลขผิด → แสดงสีแดง
                // โหมดปกติ: เทียบกับ solution | โหมด Custom: เช็คเลขซ้ำตามกฎ Sudoku
                if (playerBoard[r][c] != 0) {
                    if (!isCustomMode && playerBoard[r][c] != solution[r][c]) {
                        bg = BG_CELL_ERROR; // โหมดปกติ: ไม่ตรงเฉลย
                    } else if (isCustomMode && hasDuplicate(r, c)) {
                        bg = BG_CELL_ERROR; // โหมด Custom: เลขซ้ำในแถว/คอลัมน์/กล่อง
                    }
                }
    ...
    private void revealSolution() {
        // โหมดปกติ: เช็คว่ามีช่องที่กรอกผิดอยู่หรือไม่ → ต้องลบออกก่อนถึงจะ Solve ได้
        if (!isCustomMode) {
            for (int r = 0; r < SIZE; r++) {
                for (int c = 0; c < SIZE; c++) {
                    if (!fixed[r][c] && playerBoard[r][c] != 0 && playerBoard[r][c] != solution[r][c]) {
                        JOptionPane.showMessageDialog(this,
    ...
    // เช็คว่าผู้เล่นชนะหรือยัง: เทียบ playerBoard กับ solution ทุกช่อง
    private void checkWin() {
        // ถ้ามีช่องไหนยังไม่ตรง → ยังไม่ชนะ กลับออกไป
        for (int r = 0; r < SIZE; r++)
            for (int c = 0; c < SIZE; c++)
    ...
    // จบเกม: เมื่อผิดครบ 5 ครั้ง → ล็อคกระดาน แล้วถามว่าจะเล่นใหม่หรือดูเฉลย
    private void gameOver() {
        gameTimer.stop(); // หยุดจับเวลา
        statusLabel.setText("💀 GAME OVER");
        statusLabel.setForeground(new Color(239, 68, 68)); // สีแดง

        // ล็อคทุกช่องไม่ให้แก้ไขอีก
        for (int r = 0; r < SIZE; r++)
            for (int c = 0; c < SIZE; c++)
                cells[r][c].setEditable(false);

        // ถามผู้เล่นว่าต้องการทำอะไรต่อ
        int choice = JOptionPane.showOptionDialog(this,
                "Too many mistakes! Would you like to try again?",
                "Game Over", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE,
                null, new String[] { "New Game", "Reveal Solution" }, "New Game");

        if (choice == 0) {
            showDifficultyDialog(false); // เลือก "New Game" → เริ่มเกมใหม่
        } else {
            // เลือก "Reveal Solution" → แสดงเฉลยทั้งหมด
            for (int r = 0; r < SIZE; r++) {
                for (int c = 0; c < SIZE; c++) {
                    playerBoard[r][c] = solution[r][c];
                    cells[r][c].setText(String.valueOf(solution[r][c]));
                    if (!fixed[r][c]) {
                        cells[r][c].setForeground(ACCENT_GREEN); // สีเขียว = เฉลย
                        cells[r][c].setBackground(BG_CELL); // ลบสีแดงออก
                    } else {
                        cells[r][c].setBackground(BG_CELL_FIXED); // คืนสีช่องโจทย์
                    }
